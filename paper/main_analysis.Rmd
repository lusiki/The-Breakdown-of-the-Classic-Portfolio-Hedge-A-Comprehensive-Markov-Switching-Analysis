...
```{r regime_analysis, echo=FALSE, message=FALSE, warning=FALSE}
...
display_regime_comparison <- function(msm_obj, asset_name) {
  ...
  coefs    <- msm_obj@Coef
  stdevs   <- msm_obj@std
  transmat <- msm_obj@transMat

  # Identify regimes by volatility
  crisis_idx <- which.max(stdevs)
  normal_idx <- which.min(stdevs)

  # Build comparison table
  # Added 'check.names = FALSE' to keep column names as-is (e.g., with spaces and %)
  regime_df <- data.frame(
    `Regime`                   = c("Normal", "Crisis"),
    `Mean (% daily)`           = c(coefs[normal_idx, 1],  coefs[crisis_idx, 1]) * 100,
    `Volatility (% daily)`     = c(stdevs[normal_idx],    stdevs[crisis_idx])   * 100,
    `Persistence`              = c(transmat[normal_idx, normal_idx],
                                 transmat[crisis_idx, crisis_idx]),
    `Expected Duration (Days)` = c(1 / (1 - transmat[normal_idx, normal_idx]),
                                   1 / (1 - transmat[crisis_idx, crisis_idx])),
    check.names = FALSE
  )

  # Write the Markdown subheader
  cat("### Regime Comparison: ", asset_name, "\n\n", sep = "")

  # Render the table as HTML.
  
  kable(regime_df, digits = 3, row.names = FALSE,
        caption = paste0("Regime Characteristics for ", asset_name))  %>%
    kableExtra::kable_styling(bootstrap_options = "striped", full_width = FALSE)
}

# Display results for both models (assuming msm_spy and msm_tlt exist)
